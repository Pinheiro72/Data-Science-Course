...
```
In average the studied individual makes `r round(mean(totalSteps$stepsSum), 0)` steps each day and the median it's `r round(median(totalSteps$stepsSum), 0)`. However, this values are likely biased due to days reporting zero steps, which could indicate that the data on those days were incorrectly collected.  
```{r}
...
The dataset contains `r sum(is.na(activity$steps))` missing values. To address this issue it was imputed the 5-minute interval mean to ensure that the daily pattern isn't changed.  

After the missing value imputation, in average it's estimated that the studied individual makes `r options(scipen=999); round(mean(totalSteps2$stepsSum), 0)` steps each day and the median is `r options(scipen=999); round(median(totalSteps2$stepsSum), 0)`. Therefore, the data processing by imputing the 5-minute interval mean increased the original mean and median.  
```{r}
# graph of the total number of steps taken each day
